...
impl Vec3 {
    ...
    pub fn length_squared(&self) -> f32 {
        let x: f32x4 = Simd::from_array([self[0], self[1], self[2], 0.0]);
        let y = &x * &x;
    ...
    pub fn random_in_range(rng: &mut ThreadRng, range: RangeInclusive<f32>) -> Vec3 {
        Vec3::new(
...
pub fn dot_product(v1: &Vec3, v2: &Vec3) -> f32 {
    let x: f32x4 = Simd::from_array([v1[0], v1[1], v1[2], 0.0]);
    let y: f32x4 = Simd::from_array([v2[0], v2[1], v2[2], 0.0]);
...
pub fn cross_product(v1: &Vec3, v2: &Vec3) -> Vec3 {
    let x1: f32x4 = Simd::from_array([v1[1], v1[2], v1[0], 0.0]);
    let y1: f32x4 = Simd::from_array([v2[2], v2[0], v2[1], 0.0]);

    let z1 = x1 * y1;

    let x2: f32x4 = Simd::from_array([v1[2], v1[0], v1[1], 0.0]);
    let y2: f32x4 = Simd::from_array([v2[1], v2[2], v2[0], 0.0]);
...
pub fn refract(uv: &Vec3, n: &Vec3, e: f32) -> Vec3 {
    let cos_theta = f32::min(dot_product(&-uv, n), 1.0);
...
impl AddAssign for Vec3 {
    fn add_assign(&mut self, other: Self) {
        let x: f32x4 = Simd::from_array([self.x, self.y, self.z, 0.0]);
        let y: f32x4 = Simd::from_array([other.x, other.y, other.z, 0.0]);
...
impl Mul<Vec3> for f32 {
    ...
    fn mul(self, vec: Vec3) -> Self::Output {
        let x: f32x4 = Simd::from_array([vec.x, vec.y, vec.z, 0.0]);
        let y: f32x4 = f32x4::splat(self);
